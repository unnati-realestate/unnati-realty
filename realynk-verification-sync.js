/* REALYNK VERIFICATION SYNC V3 — broker verification + referral bonus */
(function(){
'use strict';
if(window.__REALYNK_VERIFICATION_SYNC_V3__)return;
window.__REALYNK_VERIFICATION_SYNC_V3__=true;
...
async function start(){
 ...
 const app=getApps().length?getApps()[0]:initializeApp(firebaseConfig),auth=getAuth(app),db=getFirestore(app);let stopBroker=null,stopApps=null,awardedUid='';
 ...
 onAuthStateChanged(auth,user=>{
  ...
  stopBroker=onSnapshot(doc(db,'brokers',user.uid),snap=>{broker=snap.exists()?snap.data():null;apply(broker,application,user)},err=>console.warn('Broker verification read failed',err));
  const q=query(collection(db,'brokerApplications'),where('uid','==',user.uid));
  stopApps=onSnapshot(q,snap=>{let best=null;snap.forEach(d=>{const x=d.data()||{};if(!best||statusOf(x)==='verified'||(statusOf(x)==='rejected'&&statusOf(best)==='pending'))best=x});application=best;apply(broker,application,user)},err=>console.warn('Application verification read failed',err));
 });
}
