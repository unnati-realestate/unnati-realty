/* REALYNK QUICK CATEGORY CONTROLLER — SINGLE OWNER, CAPTURED BEFORE ALL OTHER CLICK HANDLERS */
(function(){
'use strict';
...
function showHome(){var home=document.getElementById('home');if(!home)return;if(!home.classList.contains('active')){document.querySelectorAll('.screen').forEach(function(s){s.classList.toggle('active',s===home)});document.querySelectorAll('.bottom button').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-nav')==='home')})}}
function apply(scroll){
 ...
 var empty=host.querySelector('.empty');if(empty)empty.style.display='';
...
function watch(){ensureHeavy();removeLand();if(active)apply(false)}
