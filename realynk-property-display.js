/* REALYNK PROPERTY DISPLAY V5 — consistent amount + size rows for every listing type */
(function(){'use strict';
if(window.__REALYNK_PROPERTY_DISPLAY_V5__)return;window.__REALYNK_PROPERTY_DISPLAY_V5__=true;
...
function addStyle(){if(document.getElementById('realynkPropertyDisplayV5Style'))return;var st=document.createElement('style');st.id='realynkPropertyDisplayV5Style';st.textContent='.details{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:12px}.details .detail.realynk-wide{grid-column:1/-1}.details .detail b{line-height:1.3;word-break:break-word}.details .detail span{display:block;color:var(--muted);font-size:12px;font-weight:700;margin-bottom:4px}.details .detail b{font-size:16px;color:var(--navy)}';document.head.appendChild(st)}
function setDetail(box,label,value,wide){var d=document.createElement('div');d.className='detail'+(wide?' realynk-wide':'');var sp=document.createElement('span');sp.textContent=label;var b=document.createElement('b');b.textContent=value;d.appendChild(sp);d.appendChild(b);box.appendChild(d)}
...
if(t==='rent'){setDetail(box,'RENT',money(p.price),false);setDetail(box,'DEPOSIT',money(p.deposit),false)}
else if(t==='heavy deposit'){setDetail(box,'AMOUNT / DEPOSIT',money(p.price),false);if(p.deposit)setDetail(box,'DEPOSIT',money(p.deposit),false)}
else if(t==='sale'){setDetail(box,'SALE PRICE',money(p.price),false)}
else if(t==='buy'){setDetail(box,'PRICE',money(p.price),false)}
else if(t==='commercial'){setDetail(box,'PRICE / RENT',money(p.price),false)}
else{setDetail(box,'PRICE',money(p.price),false)}
setDetail(box,'CARPET / USABLE AREA',money(p.size),true);
