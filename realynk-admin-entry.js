/* REALYNK SUPER ADMIN ENTRY V4 — single-instance admin trigger */
...
/* Prevent multiple cached/versioned copies from registering competing popup requests. */
if(window.__REALYNK_ADMIN_ENTRY_V4__){
  // A newer copy is already active; do nothing.
}else{
window.__REALYNK_ADMIN_ENTRY_V4__=true;
...
let clicks=0,timer=null,loginInFlight=false;
...
function loadPanel(){
  ...
  const s=document.createElement('script');s.type='module';s.dataset.realynkSuperAdmin='1';s.src='./super-admin.js?v=8';document.head.appendChild(s);
}
function addEntry(){
  ...
  const box=document.createElement('div');box.id='realynkSuperAdminEntry';
...
async function hiddenAdminLogin(){
  ...
  try{
    ...
    if(email!==ADMIN){alert('This Google account is not the Realynk Super Admin account.');removeEntry();return;}
    addEntry();loadPanel();
  }catch(e){
    console.error('Super Admin login failed',e);
    if(e?.code!=='auth/popup-closed-by-user')alert('Super Admin login failed: '+(e?.code||e?.message||'Unknown error'));
  }finally{loginInFlight=false}
}
function watchLogo(){
  if(window.__REALYNK_ADMIN_LOGO_WATCH__ )return;
  window.__REALYNK_ADMIN_LOGO_WATCH__=true;
  document.addEventListener('click',function(e){
    ...
    if(clicks>=5){clicks=0;hiddenAdminLogin()}
  },true);
}
onAuthStateChanged(auth,u=>{
  if(String(u?.email||'').toLowerCase()===ADMIN){addEntry();loadPanel()}else removeEntry();
});
