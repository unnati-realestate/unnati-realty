/* REALYNK PROPERTY FILTER V9 — stable single owner for Home category buttons */
(function(){
'use strict';
...
function apply(){
 ...
 cards.forEach(function(c){var show=active==='All'||typeOf(c).toLowerCase()===wanted;c.style.display=show?'':'none';if(show)visible++});
...
function run(v){active=v||'All';apply();[100,500,1200].forEach(function(ms){setTimeout(apply,ms)})}
function bind(){
 injectHeavy();
 var map={buy:'Buy',sale:'Sale',rent:'Rent',commercial:'Commercial',heavyDeposit:'Heavy Deposit',clearFilter:'All'};
 Object.keys(map).forEach(function(id){var b=document.getElementById(id);if(!b)return;b.onclick=function(e){e.preventDefault();run(map[id])}});
 var s=document.getElementById('search');if(s)s.addEventListener('input',function(){active='All';setTimeout(apply,50)},false);
 var c=document.getElementById('clearFilter');if(c)c.onclick=function(e){e.preventDefault();if(s)s.value='';run('All')};
 apply();
}
...
window.realynkPropertyFilter={setFilter:run,getFilter:function(){return active},refresh:apply};
})();
