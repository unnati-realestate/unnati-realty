/* REALYNK PROPERTY FILTER V2 — reliable Buy/Sale/Rent/Commercial filtering */
(function(){'use strict';
if(window.__REALYNK_PROPERTY_FILTER_V2__)return;window.__REALYNK_PROPERTY_FILTER_V2__=true;
var active='All';
...
function message(list,show){var id='realynkFilterEmpty',m=document.getElementById(id);if(show&&!m){m=document.createElement('div');m.id=id;m.className='empty';m.textContent='No '+active.toLowerCase()+' properties available.';list.appendChild(m)}if(m)m.style.display=show?'':'none'}
function mapCards(){var list=document.getElementById('homeList');if(!list)return;var ps=props(),cards=Array.prototype.slice.call(list.children||[]).filter(function(c){return c.id!=='realynkFilterEmpty'});if(!ps.length){cards.forEach(function(c){c.style.display=''});message(list,false);return}
 ...
 var visible=cards.some(function(c){return c.style.display!=='none'});message(list,active!=='All'&&!visible)
}
