/* REALYNK AUTH ENTRY V17 — stable catalog, property rows and mobile filters */
(function(){
'use strict';
if(window.__REALYNK_AUTH_ENTRY_V17__)return;window.__REALYNK_AUTH_ENTRY_V17__=true;
/* Older boot versions used a synthetic Home-nav click after cloud restore. Block only that synthetic click so a category never jumps back. */
window.addEventListener('realynkCloudPropertiesRestored',function(){
  try{
    var home=document.querySelector('.screen.active');
    if(!home||home.id!=='home')return;
    window.__REALYNK_SUPPRESS_SYNTHETIC_HOME_NAV__=true;
    setTimeout(function(){window.__REALYNK_SUPPRESS_SYNTHETIC_HOME_NAV__=false},900);
  }catch(_){ }
},true);
window.addEventListener('click',function(e){
  try{
    if(!window.__REALYNK_SUPPRESS_SYNTHETIC_HOME_NAV__||e.isTrusted)return;
    var nav=e.target&&e.target.closest?e.target.closest('[data-nav]'):null;
    if(nav&&nav.getAttribute('data-nav')==='home'){
      e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();
      window.__REALYNK_SUPPRESS_SYNTHETIC_HOME_NAV__=false;
    }
  }catch(_){ }
},true);
