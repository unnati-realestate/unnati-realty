/* REALYNK BROKER AUTH V2 — separates broker identity from Super Admin */
...
if(!window.__REALYNK_BROKER_AUTH_V2__){
window.__REALYNK_BROKER_AUTH_V2__=true;
...
function ensureBox(){const page=document.querySelector('#account .page');if(!page)return;let box=document.getElementById('realynkBrokerAuthBox');if(!box){box=document.createElement('div');box.id='realynkBrokerAuthBox';box.className='panel';const h=page.querySelector('h2');if(h&&h.nextSibling)page.insertBefore(box,h.nextSibling);else page.prepend(box)}return box}
function render(user){const box=ensureBox();if(!box)return;if(isAdmin(user)){box.innerHTML='<b>🔐 Super Admin account</b><p class="small">Admin account is kept separate from broker profiles.</p><button id="realynkBrokerSwitch" class="primary full" type="button">👤 Continue as Broker / Switch Google Account</button>';box.querySelector('#realynkBrokerSwitch').onclick=async()=>{try{await signInWithPopup(auth,provider)}catch(e){if(e?.code!=='auth/popup-closed-by-user')alert('Broker login failed: '+(e?.code||e?.message||'Unknown error'))}}}else if(user){box.innerHTML='<b>👤 Broker Login</b><p class="small">Signed in as <strong>'+String(user.email||'').replace(/[<>]/g,'')+'</strong>. Your broker profile is connected to this account and remains Pending until Super Admin verifies it.</p><button id="realynkBrokerSignOut" class="back full" type="button">Sign out / Switch Account</button>';box.querySelector('#realynkBrokerSignOut').onclick=()=>signOut(auth).catch(e=>console.error(e))}else{box.innerHTML='<b>👤 Broker Login / Register</b><p class="small">Sign in with your Google account to connect this broker profile with Realynk. New broker accounts remain Pending until Super Admin verification.</p><button id="realynkBrokerLogin" class="primary full" type="button">Continue with Google</button>';box.querySelector('#realynkBrokerLogin').onclick=async()=>{try{const r=await signInWithPopup(auth,provider);if(isAdmin(r.user))alert('This is the Super Admin account. Please choose your broker Google account.')}catch(e){if(e?.code!=='auth/popup-closed-by-user')alert('Broker login failed: '+(e?.code||e?.message||'Unknown error'))}}}}
function hookSave(){document.addEventListener('click',async e=>{if(!e.target.closest('#saveAccount'))return;const user=auth.currentUser;if(!user||isAdmin(user))return;try{await new Promise(r=>setTimeout(r,250));await syncBroker(user)}catch(err){console.error('Broker profile sync failed',err);alert('Profile saved locally, but cloud sync failed. Please try again.')}} ,true)}
onAuthStateChanged(auth,async user=>{render(user);if(user&&!isAdmin(user)){try{await syncBroker(user)}catch(e){console.error('Broker auth sync failed',e)}}});
